...
def active_slate_date(now_et=None) -> str:
    """Return the slate date the app should display/build.

    MLB slates can run past midnight ET. Before 4 AM ET, keep using
    yesterday's slate so Reload App cannot start a new card too early.
    """
    if now_et is None:
...
def get_latest_json_file(target_date: str | None = None):
    patterns = [
    ...
    if target_date:
        dated = [p for p in files if target_date in p.name]
        if dated:
            files = dated
...
def load_latest_data():
    ...
    display_slate = active_slate_date()
    latest = get_latest_json_file(display_slate) or get_latest_json_file()
    ...
    # Overlay the locked card for the active slate. This prevents a post-midnight
    # pre-4AM refresh from showing the new calendar day before the prior slate unlocks.
    ...
    locked_final = (final_by_date.get(display_slate) or {}) if isinstance(final_by_date, dict) else {}
    locked_refined = (refined_by_date.get(display_slate) or {}) if isinstance(refined_by_date, dict) else {}
    frozen_rows = locked_final.get("rows") or []
    refined_rows = locked_refined.get("rows") or []

    if frozen_rows:
        data["final_card"] = {"generated_section": "final_card", "plays": frozen_rows}
        data["date"] = display_slate
        data.setdefault("research", {})
        if isinstance(data["research"], dict):
            data["research"]["final_card"] = frozen_rows
    if refined_rows:
        data.setdefault("research", {})
        if isinstance(data["research"], dict):
...
@app.get("/health")
def health():
    return JSONResponse({"status": "ok", "service": "hr-picks-app", "active_slate_date": active_slate_date()})
...
def refresh_data():
    ...
    with refresh_lock:
        ...
        today = active_slate_date()

        try:
            # Build/lock the active slate date. Before 4 AM ET, this is yesterday's slate.
            # That prevents post-midnight refreshes from wiping the prior slate card.
